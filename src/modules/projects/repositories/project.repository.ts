...
export class ProjectRepository {
  ...
  async deleteOne(params: WhereOptions<Project>): Promise<any> {
    return await this.projectModel.destroy({ where: params });
  }
  async updateById(
  ...
  async projectLst(body: ProjectListDto): Promise<{
    data: Array<Project>;
    total_count: number;
    total_pages: number;
  }> {
    ...
    const countQuery = `SELECT COUNT(*) AS total_count from projects`;
    if (page && limit) {
